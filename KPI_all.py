...
#Generate KPI2 for each offer, for the first 3 days of the validity period
#KPI2: a person in target group does x more transactions than a person in control group
def KPI2(population):
    transcript_file_name = 'data/transcript.json'
    ...
    for i in range(0,len(offers)):
        KPI2.append((num_trx_targ[i]/len(offer_groups[i]))/(num_trx_cont[i]/num_control))
...
def KPI4(population):
    ...
    print((num_trx_targ[0],len(offer_groups[0])),(num_trx_cont[0],num_control))
    print((num_trx_targ[1],len(offer_groups[1])),(num_trx_cont[1],num_control))
    print(offers)
    for i in range(0,len(offers)):
...
def KPI5(population):
    ...
    KPI5=[0 for i in range(0,len(offers))]
    
    for i in range(0,len(offers)):
        KPI5[i]=DollarTrxA[i]/DollarTrxB[i]
...
def KPI6(population):
    ...
    KPI6=[0 for i in range(0,len(offers))]
    for i in range(0,len(offers)):
